...
public class JdbcRepository<T : Any, ID : Any>(
    ...
    override suspend fun find(id: ID): SculkResult<T?> = io("find") {
        dataSource.connection.use { conn ->
    ...
    override suspend fun findAll(): SculkResult<List<T>> = io("findAll") {
        dataSource.connection.use { conn ->
    ...
    override suspend fun save(entity: T): SculkResult<Unit> = io("save") {
        dataSource.connection.use { conn ->
    ...
    override suspend fun delete(id: ID): SculkResult<Unit> = io("delete") {
        dataSource.connection.use { conn ->
    ...
    override suspend fun exists(id: ID): SculkResult<Boolean> = io("exists") {
        dataSource.connection.use { conn ->
    ...
    private fun columnFor(property: String): String = mapping.columns.firstOrNull { it.paramName == property }?.columnName
...
@SculkInternal
public fun <T : Any, ID : Any> jdbcRepository(dataSource: DataSource, klass: KClass<T>, dialect: SqlDialect): JdbcRepository<T, ID> =
    JdbcRepository(dataSource, OrmMapper.mappingFor(klass), dialect)
