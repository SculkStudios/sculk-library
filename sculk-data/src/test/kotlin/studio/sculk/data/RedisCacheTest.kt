...
@Serializable
private data class CachedAccount(val id: Int, val coins: Long)

class RedisCacheTest {
    @Test
    fun `save serializes into redis and find reads it back without hitting the delegate`() = runBlocking {
    ...
    @Test
    fun `delete removes the redis entry`() = runBlocking {
        val delegate = CountingRepository()
        val backend = FakeRedisBackend()
        val cache =
