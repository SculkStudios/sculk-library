...
class SculkDataTest {
    ...
    @Test
    fun `save and find round-trips correctly`() = runBlocking {
    ...
    @Test
    fun `find returns null for missing id`() = runBlocking {
        val repo = sculkData.repository<TestPlayer, Int>()
        assertNull((repo.find(999) as SculkResult.Success).value)
    }

    @Test
    fun `save is an upsert — updates existing entity`() = runBlocking {
        val repo = sculkData.repository<TestPlayer, Int>()
    ...
    @Test
    fun `exists returns correct boolean`() = runBlocking {
        val repo = sculkData.repository<TestPlayer, Int>()
    ...
    @Test
    fun `query filters orders and limits`() = runBlocking {
        val repo = sculkData.repository<TestPlayer, Int>()
        repo.saveAll(
            listOf(
                TestPlayer(1, "A", 100L),
    ...
    @Test
    fun `transaction commits on success and rolls back on failure`() = runBlocking {
        val repo = sculkData.repository<TestPlayer, Int>()
    ...
    @Test
    fun `cache serves subsequent finds and delete invalidates`() = runBlocking {
        val repo = sculkData.repository<TestPlayer, Int>()
