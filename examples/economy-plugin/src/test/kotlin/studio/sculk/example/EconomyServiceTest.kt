...
class EconomyServiceTest {
    ...
    @Test
    fun `deposit adds coins`() = runBlocking {
    ...
    @Test
    fun `withdraw rejects insufficient funds`() = runBlocking {
        assertTrue(service.withdraw(UUID.randomUUID(), "Alex", 200) is SculkResult.Failure)
    }

    @Test
    fun `transfer moves coins atomically at service level`() = runBlocking {
    ...
    @Test
    fun `pay self is rejected`() = runBlocking {
        val uuid = UUID.randomUUID()
        assertTrue(service.transfer(uuid, "Steve", uuid, "Steve", 1) is SculkResult.Failure)
    }

    @Test
    fun `negative or zero amount is rejected`() = runBlocking {
    ...
    @Test
    fun `top sorting returns highest balances first`() = runBlocking {
