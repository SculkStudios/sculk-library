...
public class StaffToolsService(
    ...
    public fun staffChatRecipients(
        viewers: Iterable<StaffViewer>,
        sender: UUID,
    ): List<StaffViewer> = viewers.filter { it.uuid == sender || it.hasStaffChatPermission }
}

public data class StaffViewer(
    val uuid: UUID,
    val name: String,
    val hasStaffChatPermission: Boolean,
)
