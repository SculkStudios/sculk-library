...
public class ServerMenus(private val settings: () -> MenuSettings) {
    public fun main(): Gui = gui(settings().title) {
        size = 27
    ...
    public fun warps(): Gui = gui("<dark_gray>Warps") {
        size = 27
        border(Material.GRAY_STAINED_GLASS_PANE) {
            name = " "
        }
    ...
    public fun profile(player: Player): Gui = gui("<dark_gray>${player.name}") {
        size = 27
        border(Material.GRAY_STAINED_GLASS_PANE) {
            name = " "
        }
    ...
    public fun onlinePlayers(): Gui = gui("<dark_gray>Online Players") {
        size = 54
        pagination {
            slots += (0 until 45)
        }
    ...
    public fun confirmNotifications(): Gui = confirmMenu("<red>Toggle Menu Tips") {
        confirm {
    ...
    public fun onlinePlayerItems(players: List<Player>): List<ItemStack> = if (players.isEmpty()) {
        listOf(
