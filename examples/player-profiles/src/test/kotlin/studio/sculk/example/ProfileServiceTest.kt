...
class ProfileServiceTest {
    ...
    @Test
    fun `first join creates a default profile`() = runBlocking {
    ...
    @Test
    fun `existing profile increments joins`() = runBlocking {
        val uuid = UUID.randomUUID()
    ...
    @Test
    fun `quit updates last seen`() = runBlocking {
        val uuid = UUID.randomUUID()
        service.loadForJoin(uuid, "Steve")
    ...
    @Test
    fun `disable flush saves loaded profiles`() = runBlocking {
        val uuid = UUID.randomUUID()
        service.loadForJoin(uuid, "Steve")
    ...
    private fun serviceFactory(): (UUID) -> PlayerProfile = { uuid ->
