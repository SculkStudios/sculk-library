...
public class CrateService(private val settings: () -> CrateSettings, private val random: (Int) -> Int = { Random.nextInt(it) }) {
    public fun validate(): List<String> = settings().crates.flatMap { (id, crate) ->
        buildList {
    ...
    public fun crate(id: String): SculkResult<CrateDefinition> = settings()
        .crates[id.normalizeId()]
        ?.let { SculkResult.success(it) }
        ?: SculkResult.failure("Unknown crate '$id'.")

    public fun keyDescriptor(crateId: String): SculkResult<ItemDescriptor> = crate(crateId).map { crate ->
        crate.keyItem.copy(data = crate.keyItem.data + (CRATE_KEY to crateId.normalizeId()))
    }

    public fun keyItem(crateId: String): SculkResult<ItemStack> = keyDescriptor(crateId).flatMap { descriptor ->
        descriptor.toItemStack()?.let { SculkResult.success(it) }
            ?: SculkResult.failure("Crate key material '${descriptor.material}' is invalid.")
    }

    public fun isCrateKey(stack: ItemStack?, crateId: String): Boolean {
    ...
    public fun roll(crateId: String): SculkResult<CrateReward> = crate(crateId).flatMap { crate ->
