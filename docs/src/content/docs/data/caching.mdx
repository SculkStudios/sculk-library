...
sculk.getEvents().listen(PlayerJoinEvent.class, event -> {
    UUID uuid = event.getPlayer().getUniqueId();
    sculk.getScheduler().runAsync(() ->
        healthCache.findOrCreate(uuid, () ->
            new PlayerHealth(uuid, 10, 10)
        )
    );
});
```
...
```java
sculk.getScheduler().runAsync(() -> {
    SculkResult<List<PlayerShards>> top = shardsCache.findTopBy(
        10,
        PlayerShards::getShards,
        true  // descending
    );
});
```
...
```java
sculk.getScheduler().runAsync(() -> {
    List<PlayerData> updated = buildUpdatedList();
    SculkResult<Void> result = cache.saveAll(updated);
    if (result instanceof SculkResult.Failure<?> f) {
        logger.warning("Batch save failed: " + f.getMessage());
    }
});
```
</TabItem>
