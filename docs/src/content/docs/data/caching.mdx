...
```kotlin
sculk.scope.launchAsync {
    val topShards = repository.topBy("shards", rows = 10).getOrThrow()
    // topShards is List<PlayerShards>, descending by default

    val bottomHearts = repository.topBy("hearts", rows = 5, ascending = true).getOrThrow()
}
```

:::note
`topBy` sorts and limits **in SQL**, so it stays cheap on a table with a million rows. The previous cache-side version loaded the whole table and sorted it in memory, which is survivable at a thousand rows and not at a million.
:::
...
- **Use `topBy` for leaderboards** — it is always fresh, and the database does the sorting.
