...
The config system maps YAML files to Kotlin data classes. Absent keys are written automatically on first load, and reads stay typed instead of relying on repeated `getString("some.key")` calls.
