...
The config system maps YAML files to Kotlin data classes. Missing keys are written automatically on first load, and reads stay typed instead of relying on repeated `getString("some.key")` calls.
...
Configs can define a `configVersion` field and register migration steps before loading. Migrations edit the raw YAML map, then Sculk writes any new defaults normally.
...
sculk.config.migrations<Settings> {
    from(1).to(2) {
        rename("spawn-world-name", "spawn-world")
...
Register migrations before `load<Settings>()`. If a config is already loaded, Sculk rejects late migration registration so startup behavior stays predictable.
...
@ConfigFile("settings.yml")
data class Settings(
    @param:Min(1) @param:Max(50) val maxHomes: Int = 5,
    @param:NotEmpty val prefix: String = "<gray>[<aqua>Plugin<gray>] ",
)
```
</TabItem>
<TabItem label="Java">
...
Sculk validates constructor parameter annotations. In Kotlin, use the explicit `@param:` target for `@Min`, `@Max`, and `@NotEmpty`; Java records do not need a use-site target.

Invalid values are reported and the loaded object keeps the field's default where Sculk can safely do so. Treat validation messages as configuration errors to fix, not as something to ignore permanently.
...
## Self-documenting configs with @Comment

Annotate any field with `@Comment` to emit comment lines directly above that key in the generated YAML. Server operators can understand every setting without ever reading source code.
...
data class CombatSettings(
    ...
    @Comment("How long the combat tag lasts in milliseconds. 1 second = 1000 ms")
    ...
    @Comment("Name of the WorldGuard region where PvP is always allowed")
    val pvpRegion: String = "pvp",
)
```

Generated `settings/combat.yml`:
```yaml
# How long the combat tag lasts in milliseconds. 1 second = 1000 ms
...
# Name of the WorldGuard region where PvP is always allowed
pvp-region: pvp
```
</TabItem>
<TabItem label="Java">
...
public record CombatSettings(
    ...
    public CombatSettings() { this(30_000L, 5, 15); }
}
```
</TabItem>
</Tabs>

Multi-line comments are supported — separate lines with `\n` in the annotation value:

```kotlin
@Comment("Kill cooldown in milliseconds.\nPrevents kill-farming the same player repeatedly.\n1 minute = 60000 ms")
val killCooldown: Long = 60_000L,
```

`@Comment` can be combined with `@param:Min`, `@param:Max`, and `@param:NotEmpty` on the same Kotlin property.

## Best practices

- One `@ConfigFile` per logical group: `settings.yml`, `messages.yml`, `economy.yml`.
- Use subdirectories to keep configs tidy: `@ConfigFile("settings/combat.yml")`, `@ConfigFile("messages/combat.yml")`.
- Keep defaults sensible for a fresh server install — operators should never need to touch the file just to get running.
- Use `@param:Min`/`@param:Max` on any Kotlin numeric field where an out-of-range value would cause a bug.
- Use `@Comment` on every field — a config file that explains itself is a config file that never gets misconfigured.
