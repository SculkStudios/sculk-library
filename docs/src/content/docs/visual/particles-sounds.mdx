...
`sculk-visual` wraps Bukkit's particle and sound APIs in clean, readable builders. Instead of calling `world.spawnParticle(...)` with six positional arguments, you use a DSL that names everything.
...
For multi-step animations spread across multiple ticks, see [Timelines & Sequences](/visual/timelines/).
