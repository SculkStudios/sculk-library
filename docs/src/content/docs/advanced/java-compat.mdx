...
Sculk Studio is Kotlin-first but Java is a first-class citizen. Every Kotlin DSL entry point has a paired fluent builder in a `studio.sculk.*.java` sub-package. No generics in public APIs, no Kotlin-only return types, no gotchas.
